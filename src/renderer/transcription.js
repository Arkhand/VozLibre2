...
(function () {
  ...
  // Devuelve { text, language } — language es el idioma que Whisper DETECTÓ en el
  // audio (código ISO: "en", "es"…), no el que pedimos. Sirve para avisar cuando el
  // audio no era el idioma configurado y para que el formateador no traduzca.
  ...
  async function postWithRetry(url, apiKey, form) {
    for (let intento = 0; ; intento++) {
      ...
      try {
        ...
        if (intento >= RETRY_WAITS.length) throw e;
        await esperar(RETRY_WAITS[intento], intento, "sin conexión");
        continue;
      }
      ...
      await esperar(espera, intento, res.status === 429 ? "límite de cuota" : "error " + res.status);
    }
  }

  function esperar(ms, intento, motivo) {
    const seg = Math.ceil(ms / 1000);
    cb.onStatus(`Groq: ${motivo}. Reintentando en ${seg} s… (${intento + 1}/${RETRY_WAITS.length})`);
  ...
  // Transcribe UN trozo y DEVUELVE {text, language} (null si falló), sin tocar el
